...
// v2.0.0.0.1 Pinned: asynchronous intermediate D2H/H2D through one CUDA host-buffer arena.
// Falls back to per-entry std::vector storage if pinned host memory is unavailable.
...
struct ggml_backend_sched_tp_pinned_owner {
    ggml_backend_buffer_t buffer = nullptr;

    ggml_backend_sched_tp_pinned_owner() = default;
    ggml_backend_sched_tp_pinned_owner(const ggml_backend_sched_tp_pinned_owner &) = delete;
    ggml_backend_sched_tp_pinned_owner & operator=(const ggml_backend_sched_tp_pinned_owner &) = delete;

    ~ggml_backend_sched_tp_pinned_owner() {
        if (buffer != nullptr) {
...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched, int turboprefill_stage) {
    ...
    static constexpr const char * GGML_BACKEND_SCHED_TP_FILE_VERSION =
            "TurboPrefill_b10068_v2.0.0.0.1_Pinned";
    ...
    if (turboprefill_stage != 0) {
        ...
        ggml_backend_sched_tp_pinned_owner intermediate_pinned_arena;
        {
            ...
            if (host_buft != nullptr) {
                intermediate_pinned_arena.buffer = ggml_backend_buft_alloc_buffer(host_buft, arena_size);
                if (intermediate_pinned_arena.buffer != nullptr &&
                    ggml_backend_buffer_get_type(intermediate_pinned_arena.buffer) != host_buft) {
                    // The CUDA host-buffer backend can return a regular CPU buffer on allocation failure.
                    ggml_backend_buffer_free(intermediate_pinned_arena.buffer);
                    intermediate_pinned_arena.buffer = nullptr;
                }
            }

            void * arena_data = intermediate_pinned_arena.buffer != nullptr
                    ? ggml_backend_buffer_get_base(intermediate_pinned_arena.buffer)
                    : nullptr;
